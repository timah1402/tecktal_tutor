...
所有步骤完成后，把最终答案写成一份完整的分步讲解，讲给一个从未学过这部分内容的人听：先清楚给出精确结果，然后按顺序回顾每一步——做了什么、为什么这一步是对的、它和前后步骤之间是什么关系。凡是初学者可能不认识的术语或记号都要解释清楚。不要为了简洁而压缩或跳过步骤，也不要假设对方已经具备相关背景——这是一次教学式的讲解，而不是简短总结。如生成了图形/文件，一并呈现。
