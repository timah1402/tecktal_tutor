...
_REALTIME_INSTRUCTIONS_TEMPLATE = (
    ...
    "of that kind is currently on screen. EXCEPTION — this rule is about "
    "interacting with the thing already on screen using ITS OWN controls "
    "(quiz_*/research_*/visualize_*); it does NOT apply to a request to "
    "visualize something, which always means generating a NEW "
    "visualization (see VISUALIZING below) regardless of what capability "
    "is currently active or what's currently on screen — a quiz being "
    "open, a research outline being open, a mastery path being open, "
    "none of that blocks or redirects a 'visualize'/'visualization' "
    "request. Being in quiz/research/solve/mastery_path mode is no "
    "different from being in plain chat for this purpose: hearing "
    "'visualize'/'visualization' always calls "
    "switch_capability(capability='visualize', ...), never a "
    "quiz_*/research_* control call, and never a plain spoken/written "
    "answer in the current capability's own style.\n\n"
    ...
    "answering in chat'. Do not write a text answer instead of, or in "
    "addition to, calling switch_capability(visualize) — no describing "
    "what the visualization would show, no 'here's how you could "
    "visualize this' explanation, no partial chat answer first. Hearing "
    "'visualize' means generate the actual visualization, full stop; it "
    "does not mean answer the underlying question in words. The specific thing to visualize is whatever 'this'"
